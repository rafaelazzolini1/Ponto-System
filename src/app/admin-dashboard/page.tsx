...
export default function AdminDashboard() {
  // ✅ useAuth atualizado: não usa mais js-cookie internamente
  ...
  const [filters, setFilters] = useState<DashboardFilters>({
    dataInicio: DateTime.now().setZone('America/Sao_Paulo').toFormat('yyyy-MM-dd'), // ✅ data local de Brasília
    dataFim: '',
    cpf: '',
    nome: '',
    status: 'todos',
  ...
  const applyFilters = () => {
    ...
    if (filters.cpf) filtered = filtered.filter(e => e.cpf.includes(filters.cpf));
    if (filters.nome) filtered = filtered.filter(e => e.nome.toLowerCase().includes(filters.nome.toLowerCase()));
    if (filters.status !== 'todos') filtered = filtered.filter(e => e.statusAtual === filters.status);
      ...
      <ProtectedRoute requiredRole="admin">
        <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-lg p-6 text-center w-full max-w-md">
            ...
            <p className="text-gray-600 text-sm">Coletando dados dos funcionários.</p>
          </div>
        </div>
      </ProtectedRoute>
    );
          ...
          <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6 mb-4 sm:mb-6">
            <div className="flex flex-col gap-4">
              <div>
                ...
                {/* ✅ user.nome vem agora via query Firestore pelo uid — sem cookies JS */}
                    ...
                    <div className="bg-white rounded-xl shadow-lg p-4 sm:p-6">
                      ...
                      <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={dadosGraficoHoras}>
                          ...
                          <XAxis dataKey="nome" angle={-45} textAnchor="end" height={60} fontSize={9} interval={0} />
                          <YAxis tickFormatter={horasTickFormatter} fontSize={10} />
                          <Tooltip formatter={horasTooltipFormatter} labelFormatter={(label) => `Funcionário: ${label}`} />
              ...
              <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                <div className="p-3 sm:p-4 lg:p-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                    <div>
                      ...
                      <input type="date" value={filters.dataInicio} onChange={(e) => setFilters({ ...filters, dataInicio: e.target.value })} className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 text-xs sm:text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Data Fim</label>
                      <input type="date" value={filters.dataFim} onChange={(e) => setFilters({ ...filters, dataFim: e.target.value })} className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 text-xs sm:text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">CPF</label>
                      <input type="text" placeholder="Filtrar por CPF" value={filters.cpf} onChange={(e) => setFilters({ ...filters, cpf: e.target.value })} className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 text-xs sm:text-sm" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Nome</label>
                      <input type="text" placeholder="Filtrar por nome" value={filters.nome} onChange={(e) => setFilters({ ...filters, nome: e.target.value })} className="w-full px-2 sm:px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 text-xs sm:text-sm" />
                    </div>
                  </div>
                </div>
                ...
                <div className="p-3 sm:p-4 lg:p-6 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-base sm:text-lg lg:text-xl font-semibold text-gray-800">Funcionários</h2>
                  </div>
                  <button type="button" onClick={handleRefresh} className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-3 sm:px-4 py-2 rounded-lg hover:from-blue-700 hover:to-blue-800 transition-all duration-200 transform hover:-translate-y-0.5 hover:shadow-lg flex items-center gap-2 text-xs sm:text-sm w-full sm:w-auto justify-center">
                      ...
                      <table className="w-full">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            {!isInterval && <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>}
                            <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Horas Trabalhadas</th>
                            <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{isInterval ? "Soma Horas Extras" : "Horas Extras"}</th>
                            {!isInterval && (
                              <>
                                <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Entrada Inicial</th>
                                <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Saída Inicial</th>
                                <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Entrada Final</th>
                                <th className="px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Saída Final</th>
                              </>
                            )}
                          </tr>
                        </thead>
                          ...
                          {filteredEmployees.map((employee) => {
                            ...
                            return (
                              <tr key={employee.cpf} className="hover:bg-gray-50">
                                <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{employee.nome}</td>
                                {!isInterval && (
                                  <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                                    <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(employee.statusAtual)}`}>{getStatusText(employee.statusAtual)}</span>
                                  </td>
                                )}
                                <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{employee.horasTrabalhadasPeriodo}</td>
                                <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm font-medium">
                                  {isInterval ? (
                                    <div className="text-gray-600">{calcularSomaHorasExtras(employee.registrosPeriodo)}</div>
                                  ) : (
                                    <div className={`${employee.horasTrabalhadasMinutos > (8 * 60 + 48) ? 'text-red-600' : 'text-gray-400'}`}>{calcularHorasExtras(employee.horasTrabalhadasMinutos)}</div>
                                  )}
                                </td>
                                {!isInterval && (
                                  <>
                                    <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{entradaInicial ? formatarHora(entradaInicial.timestamp) : '-'}</td>
                                    <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{saidaInicial ? formatarHora(saidaInicial.timestamp) : '-'}</td>
                                    <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{entradaFinal ? formatarHora(entradaFinal.timestamp) : '-'}</td>
                                    <td className="px-4 lg:px-6 py-4 whitespace-nowrap text-sm text-gray-500">{saidaFinal ? formatarHora(saidaFinal.timestamp) : '-'}</td>
                                  </>
                                )}
                              </tr>
                            );
                        ...
                        return (
                          <div key={employee.cpf} className="p-4 hover:bg-gray-50">
                            <div className="flex justify-between items-start mb-3">
                              <h3 className="font-semibold text-gray-900 text-sm">{employee.nome}</h3>
                              {!isInterval && (
                                <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${getStatusColor(employee.statusAtual)}`}>{getStatusText(employee.statusAtual)}</span>
                              )}
                            </div>
                            <div className="grid grid-cols-2 gap-3 text-xs">
                              ...
                              <div className="bg-gray-50 p-2 rounded">
                                ...
                                {isInterval ? (
                                  <span className="font-medium text-gray-600">{calcularSomaHorasExtras(employee.registrosPeriodo)}</span>
                                ) : (
                                  <span className={`font-medium ${employee.horasTrabalhadasMinutos > (8 * 60 + 48) ? 'text-red-600' : 'text-gray-400'}`}>{calcularHorasExtras(employee.horasTrabalhadasMinutos)}</span>
                                )}
                              </div>
                            </div>
                            {!isInterval && (
                              <div className="mt-3 grid grid-cols-4 gap-2 text-xs text-center border-t border-gray-100 pt-3">
                                <div><span className="block text-gray-400 mb-1">Ent 1</span><span className="text-gray-700">{entradaInicial ? formatarHora(entradaInicial.timestamp) : '-'}</span></div>
                                <div><span className="block text-gray-400 mb-1">Sai 1</span><span className="text-gray-700">{saidaInicial ? formatarHora(saidaInicial.timestamp) : '-'}</span></div>
                                <div><span className="block text-gray-400 mb-1">Ent 2</span><span className="text-gray-700">{entradaFinal ? formatarHora(entradaFinal.timestamp) : '-'}</span></div>
                                <div><span className="block text-gray-400 mb-1">Sai 2</span><span className="text-gray-700">{saidaFinal ? formatarHora(saidaFinal.timestamp) : '-'}</span></div>
                              </div>
                            )}
                          </div>
                        );
